...
class MCANet_C(nn.Module):

    def __init__(self, n_channels, n_classes, bilinear=True):
        super(MCANet_C, self).__init__()
    ...
    def forward(self, x):
        ...
        x6 = self.up1(x5, x4)
        x6_scale = F.interpolate(x6,
                                 size=[256, 256],
                                 mode='bilinear',
                                 align_corners=True)

        x7 = self.up2(x6, x3)
        x7_scale = F.interpolate(x7,
                                 size=[256, 256],
                                 mode='bilinear',
                                 align_corners=True)

        x8 = self.up3(x7, x2)
        x8_scale = F.interpolate(x8,
                                 size=[256, 256],
                                 mode='bilinear',
                                 align_corners=True)
        ...
        x9 = torch.cat((x9, x8_scale, x7_scale, x6_scale, x5_scale), 1)
...
if __name__ == '__main__':
    model_ = MCANet_C(n_channels=3, n_classes=1)
